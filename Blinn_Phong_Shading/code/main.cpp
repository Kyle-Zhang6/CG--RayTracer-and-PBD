...
float compensation = 200, compensation2 = 0.5;
...
Vector3f getColor(Vector3f p, Vector3f n, Vector3f u, Vector3f v)
{
	...
	for (auto& light : lights)
	{
		...
		if (light.type == POINT)
		{
			// Because 'Blinn-Phong Model' take distance into consideration, while in this 
			// program, the sphere is supposed to fill the most part of the created window.
			// Thus when we are creating lights, they can easily be too far away so that both
			// diffusion light and specular light could be very weak when reaching the surface
			// of the sphere. As a consequence, I apply a parameter 'compensation' to compensate
			// the weakend light.
			...
			float r2 = l.norm2();
			l.normalize();
			Ld += kd * (light.intensity / r2)*max(0.0f, n.dot(l))*compensation; // Diffusion light
			...
			Lsu = ks * (light.intensity / r2)*pow(max(0.0f, n.dot(hu)), pu)*compensation;
			Lsv = ks * (light.intensity / r2)*pow(max(0.0f, n.dot(hv)), pv)*compensation;
			Ls += clamp(Lsu*Lsv); // Multiply specular reflection on both direction to get the final specular term
		}
		else if(light.type == DIRECTIONAL)
		{
			l = -normalize(light.pos);// For a directional light, its direction is opposite to 'l'
			Ld += kd * (light.intensity)*max(0.0f, n.dot(l));
			Vector3f e = eyePos - p;
			e.normalize();
			Vector3f h = normalize(e + l);
			Vector3f hu = normalize(projection(h, u) + projection(h, n));
			Vector3f hv = normalize(projection(h, v) + projection(h, n));
...
Vector3f getColor(Vector3f p, Vector3f n)
{
	...
	for (auto& light : lights)
	{
		...
		if (light.type == POINT)
		{
			...
			float r2 = l.norm2();
			l.normalize();
			Ld += kd * (light.intensity / r2)*max(0.0f, n.dot(l))*compensation;

			Vector3f e = eyePos - p;
			e.normalize();
			Vector3f h = normalize(e + l);
			Ls += ks * (light.intensity / r2)*pow(max(0.0f, n.dot(h)),puv)*compensation;
		}
		else
		{
			...
			e.normalize();
			Vector3f h = normalize(e + l);
			Ls += ks * (light.intensity)*pow(max(0.0f, n.dot(h)),puv)*compensation2;
		}
	}
...
int main(int argc, char *argv[]) 
{
	...
	//lights.push_back(Light(POINT, { (float)Width_global / 2,1000,0 }, { 1000,1490,2370 }));
