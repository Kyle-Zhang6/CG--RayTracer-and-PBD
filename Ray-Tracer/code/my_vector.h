...
public:
	...
	Vector3f operator *= (const float& r) const { return r * (*this); }
	Vector3f operator *= (const Vector3f& v) const { return v * (*this); }
	Vector3f operator /= (const float& r) const { return (*this) / r; }

	inline float dot(const Vector3f& v) const { return x * v.x + y * v.y + z * v.z; }
	inline Vector3f cross(const Vector3f& v) const { return Vector3f(y*v.z - z * v.y, z*v.x - x * v.z, x*v.y - y * v.x); }
...
inline Vector3f clamp0(Vector3f v)
{
	return Vector3f(std::max(v.x, 0.0f), std::max(v.y, 0.0f), std::max(v.z, 0.0f));
}
inline Vector3f projection(Vector3f a, Vector3f b)
{
	b.normalize();
	return a.dot(b) * b;
}

// Specify aliases for Vector3f
